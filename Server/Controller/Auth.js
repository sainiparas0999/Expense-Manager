...
const { passwordUpdated } = require("../templates.js/PaswordUpdate");
...
exports.signup = async (req, res) => {
	try {
		...
		const BudgetDetails = await Budget.create({
			 totalBudget:null,
			 Income:null,
		});
		...
		const FinancialGoalsDetails = await FinancialGoals.create({
			BankBalance:null,
			Goal:null
		});

		console.log(FinancialGoalsDetails);
		...
		const user = await User.create({
			...
			Budget:BudgetDetails._id ,
			FinancialGoals:FinancialGoalsDetails._id,
...
exports.sendotp = async (req, res) => {
	try {
        ...
        console.log("function call gyi hAI")
 ...
 exports.changePassword = async (req,res) =>{
	...
	try{
        // collect the data from the req.body 
	...
	// validate the confirmPassword and NewPassword 
	...
	if(confirmPassword !== NewPassword){
	   ...
	   if(!isPassword){
		return res.status(400).json({
			success:false ,
			message:"Please fill the correct oldPassword",
		})
	   }
